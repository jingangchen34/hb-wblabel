...
import { OccGridData } from './occ';
import { buildPointLabelColors, setPointLabelByIndices } from './occ/pointLabel';
import OccGridLayer from './occ/OccGridLayer';
...
export default class PointCloud extends THREE.EventDispatcher {
    ...
    groupOcc: THREE.Group;
    groupTrack: THREE.Group;
    occLayer: OccGridLayer;
    ...
    constructor() {
        ...
        this.groupOcc = new THREE.Group();
        this.groupTrack = new THREE.Group();
        this.occLayer = new OccGridLayer();
        this.groupOcc.add(this.occLayer);
    ...
    setOccGridData(data: OccGridData) {
        this.occLayer.setData(data);
        this.render();
    }

    clearOccGrid() {
        this.occLayer.clearLayer();
        this.render();
    }

    setOccOpacity(opacity: number) {
        this.occLayer.setOpacity(opacity);
        this.render();
    }

    setOccZRange(range?: [number, number]) {
        this.occLayer.setZRange(range);
        this.render();
    }

    setOccVisibleLabels(labels: number[]) {
        this.occLayer.setVisibleLabels(labels);
        this.render();
    }

    setOccVoxelLabel(x: number, y: number, z: number, label: number) {
        this.occLayer.setVoxelLabel(x, y, z, label);
        this.dispatchEvent({ type: 'occ_edit', data: this.occLayer.exportPatch() });
        this.render();
    }

    exportOccPatch() {
        return this.occLayer.exportPatch();
    }

    setPointLabelByIndices(indices: number[], label: number, colorMap?: Record<number, string>) {
        const points = this.groupPoints.children[0] as Points;
        if (!points) return undefined;
        const labels = setPointLabelByIndices(points, indices, label, colorMap);
        this.render();
        return labels;
    }

    setPointLabels(labels: Uint8Array, colorMap?: Record<number, string>) {
        const points = this.groupPoints.children[0] as Points;
        if (!points) return;
