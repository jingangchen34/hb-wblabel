...
def find_clip_dirs(root: Path, require_obstacle: bool = True) -> list[Path]:
    clips: list[Path] = []
    for dirpath, dirnames, _ in os.walk(root):
        current = Path(dirpath)
        pose = current / "pose.json"
        has_obstacle = find_obstacle_file(current) is not None
        if pose.is_file() and (has_obstacle or not require_obstacle) and find_lidar_bins(current):
...
def generate_sql(args: argparse.Namespace) -> tuple[str, int, int]:
    ...
    clips = find_clip_dirs(scan_root, require_obstacle=not args.skip_obstacle_annotations)
    vargen = VarGen()
    lines: list[str] = [
