...
public class PointCloudUploadUseCase {
    ...
    public List<String> getDataNames(File sceneFile) {
        ...
        for (var f : sceneFile.listFiles()) {
            ...
            if (boo) {
                var list = Arrays.stream(f.listFiles())
                        .filter(fl -> Constants.PCD_SUFFIX.equalsIgnoreCase(FileUtil.getSuffix(fl))
                                || Constants.BIN_SUFFIX.equalsIgnoreCase(FileUtil.getSuffix(fl)))
                        .map(uploadDataUseCase::getFilename).collect(Collectors.toSet());
                sceneNames.addAll(list);
            } else if (f.isDirectory() && "lidars".equals(filename)) {
                var list = Arrays.stream(f.listFiles())
                        .filter(File::isDirectory)
                        .flatMap(sensorDir -> Arrays.stream(sensorDir.listFiles()))
                        .filter(fl -> Constants.BIN_SUFFIX.equalsIgnoreCase(FileUtil.getSuffix(fl)))
                        .map(uploadDataUseCase::getFilename).collect(Collectors.toSet());
                sceneNames.addAll(list);
            }
        }
    ...
    private void getPointCloudParentFile(File file, Set<File> pointCloudParentList) {
        ...
        } else if ("lidars".equals(filename) && FileUtil.isDirectory(file)) {
            pointCloudParentList.add(file.getParentFile());
        }
    }
}
